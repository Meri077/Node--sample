...
exports.dealer_user_cache = function(app, path, ...middleWare) {
    restMethods.selected('g', app, path, 'ep_dealer_user_cache',[
        ...
        { name : 'do_not_text_request_date', type : 'date', allowNull : true },
    ], undefined, ...middleWare);

    app.get(path, ...middleWare, function (req, res) {
        dealerUserCacheApi.getAll(function(err, result) {
    ...
    app.post(path + '/get_number_of_credit_applications', ...middleWare, function(req, res) {
        dealerUserCacheApi.getNumberOfCreditApplications(req.body.uuid, function(err, result) {
            if (err) return res.send(err);
            return res.send(result);
        });
    });

    app.post(path + '/get_dealer_stats', ...middleWare, function(req, res) {
        dealerUserCacheApi.getDealerStats(function(err, result) {
            if (err) return res.send(err);
            return res.send(result);
        });
    });
    ...
    app.post(path + '/:id/delete_user', ...middleWare, app._hasRole.manager, function (req, res) {
        dealerUserCacheApi.delete_user(req.params.id, function(err, result) {
            if (err) return res.status(404).send('User Cache not Found');
            return res.send(result);
        });
    });

    app.post(path + '/:id/update_do_not_text_request_date', ...middleWare, app._hasRole.manager, function (req, res) {
        dealerUserCacheApi.update_do_not_text_request_date(req.params.id, req.body.updateData, function(err, result) {
            if (err) return res.status(404).send('User Cache not Found');
            return res.send(result);
        });
    });
